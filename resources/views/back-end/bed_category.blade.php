...
@section('brad-title','Bed Category list')
@section('content')
    <div class="breadcrumb-area d-flex justify-content-between">
        <h4 class="h4 text-bold">Bed Category List</h4>
        <button href="#" class="btn btn-primary btn-sm float-end"  data-toggle="modal"
                data-target="#staticBackdrop">Add Bed Category</button>
    </div>
        ...
        <div class="modal-dialog modal-md" style="max-width: 60%">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Bed Category Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="modal-form"  action="{{route('bc.store')}}" method="post">
                    @csrf
                    <div class="modal-content">
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Name</label>
                                <select name="bed_floor_id" id="" class="form-control">
                                    <option value="" selected disabled>-- Select A Floor --</option>
                                @forelse($floor as $f)
                                    <option value="{{$f->id}}" >{{ $f->name }}</option>
                                    @empty
                                        <option disabled>-- No Floor Found--</option>
                                    @endforelse
                                </select>
